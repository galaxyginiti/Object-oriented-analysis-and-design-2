...
/// <summary>
/// ConcreteBuilder: Дизайнер — Portfolio обязателен, авто-добавляет Figma и Adobe CC
/// </summary>
public class DesignResumeBuilder : IResumeBuilder
{
    ...
    {
        _skills.AddRange(s);
        if (!_skills.Contains("Figma")) _skills.Add("Figma");       // Design-специфика
        if (!_skills.Contains("Adobe CC")) _skills.Add("Adobe CC"); // Design-специфика
    }
    ...
    {
        _github = gh;
        _portfolio = !string.IsNullOrWhiteSpace(pf) ? pf : "https://behance.net/"; // Portfolio обязателен
        _linkedin = li;
    }
...
/// <summary>
/// ConcreteBuilder: Менеджер — LinkedIn обязателен, авто-добавляет Leadership и Agile/Scrum
/// </summary>
public class ManagerResumeBuilder : IResumeBuilder
{
    private string _name = "", _email = "", _phone = "", _summary = "";
    private List<Experience> _exps = new();
    private List<Education> _edus = new();
    ...
    {
        _skills.AddRange(s);
        if (!_skills.Contains("Leadership")) _skills.Add("Leadership");   // Manager-специфика
        if (!_skills.Contains("Agile/Scrum")) _skills.Add("Agile/Scrum"); // Manager-специфика
    }
    ...
    public void SetLinks(string? gh, string? pf, string? li)
    {
        _github = gh;
        _portfolio = pf;
        _linkedin = !string.IsNullOrWhiteSpace(li) ? li : "https://linkedin.com/in/"; // LinkedIn обязателен
    }
